...
import { applyAndSaveCoupon, isCouponValid } from "../../utils/couponValidation.js";
...
export const updateCoupon = errorHandler(async (req, res, next) => {
  ...
  if (couponCode) {
    if (coupon.couponCode == couponCode.toLowerCase()) {
      return next(new AppError("coupon code match old coupon code plz change it", 401));
    }
    if (await couponModel.findOne({ couponCode: couponCode.toLowerCase() })) {
      return next(new AppError("coupon code is already exist", 401));
    }
  }
  const updatedCoupon= await couponModel.findByIdAndUpdate(req.params._id,{
    updatedBy:_id,
    ...req.body
  },{new:true})
...
export const getAllCoupons = errorHandler(async (req, res, next) => {
  ...
  return res.status(200).json({message:"done" ,count:coupons.length,coupons})
});

export const applyCoupon = errorHandler(async (req, res, next, model, modelName) => {
  const { couponCode } = req.body;
  const { _id } = req.user;
  // Find the coupon by its code
  const coupon = await couponModel.findOne({ couponCode: couponCode.toLowerCase() });
