...
export const addReview = errorHandler(async (req, res, next) => {
    ...
    if (!allowRev) {
      return next(new AppError("You must buy this product before reviewing it.", 401));
    }
    ...
    if (isRevBefore) {
      return next(new AppError("You have already reviewed this product before.", 400));
    }
...
export const updateReview = errorHandler(async (req, res, next) => {
    ...
    if (!review) {
      return next(new AppError("Rmade eview not found or you don't have permission to update it.", 404));
    }
    updateRating(review.reviews,review)
...
export const getProductReviews= errorHandler(async(req,res,next)=>{
  ...
  const apiFeaturesInstance= new ApiFeatures(reviewModel.find({productId, "reviews": {
    $elemMatch: {
      "rating": { $in: [...parse] }
    }}}),req.query)
  .pagination()
  ...
  return res.status(200).json({message:"Done", count:reviews.length,reviews})
})