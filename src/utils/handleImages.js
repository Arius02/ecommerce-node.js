...
export async function handleImagesUpdateAndDelete(_id,files, imagesToDelete, product, path,next) {
    const check=await productModel.findOne({
      ...
      if(files.length !==imagesToDelete.length){
        return next(new AppError("you should upload and delete the same number of photos . ",400))
      }
